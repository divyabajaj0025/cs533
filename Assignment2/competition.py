...
def evaluate_policy_discounted(env, policy, discount_factor, trials = 1000):
    ...
    for _ in range(trials):
        ...
        while not done:
            observation, reward, done, info = env.step(policy[observation])
            total_reward += (pow(discount_factor, i)) * reward
            i += 1
    return total_reward / trials
...
def VI_worker_v3(VI_server, data, update_range):
        ...
        VI_server.update.remote(update_range, v_new, p_new, error)
...
v_np, pi_np  = np.array(v), np.array(pi)
end_time = time.time()
run_time['Fast value iteration'] = end_time - start_time
print("time:", run_time['Fast value iteration'])
print_results(v, pi, map_size, env, beta, 'fast_vi')
